...
import { getCachedPlannedTrips } from '@/lib/tripCache'
  ...
  return (
    <section className="mb-14">
      ...
      {cards.length > 0 ? (
          ...
          {cards.map((card) => (
            ...
            >
              <div className="relative w-full h-24 rounded-xl overflow-hidden mb-3">
                ...
                {card.isCached && (
                  <span className="absolute top-2 left-2 inline-flex items-center rounded-full bg-black/45 text-white text-[10px] font-semibold px-2 py-1 tracking-wide">
                    Cached
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">{card.duration ? `${card.duration} days` : 'Duration N/A'} · {card.price}</p>
              <p className="text-[11px] text-coral/90 mt-1 font-semibold">{card.packageTitle}</p>
              {card.cachedAtLabel && (
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">Saved {card.cachedAtLabel}</p>
              )}
        ...
        <div className="rounded-2xl border border-cream-dark/70 dark:border-white/10 bg-white/70 dark:bg-white/5 p-6 text-sm text-slate-500 dark:text-slate-400">
          No saved trips yet. Plan a trip and it will appear here on this browser.
        </div>
      )}
    </section>
  )
...
function DestinationsPage() {
  ...
  useEffect(() => {
    setCachedTrips(getCachedPlannedTrips())
  }, [])

  const cachedCards = useMemo(() => {
    return cachedTrips.map((trip) => ({
      tripId: trip.tripId,
      city: trip.city,
      country: trip.country,
      duration: trip.duration || 0,
      price: trip.budget || 'Not set',
      image: trip.image,
      packageTitle: trip.packageTitle || 'Previously Planned Trip',
      isCached: true,
      cachedAtLabel: trip.cachedAt
        ? new Date(trip.cachedAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })
        : '',
    }))
  }, [cachedTrips])
      ...
      <div className="min-h-screen bg-cream dark:bg-[#0B1120] transition-colors duration-500 pt-28 pb-16 px-5 sm:px-8 lg:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          <DestinationGrid
            ...
            subtitle="Trips saved on this browser cache. Available after reload and kept for up to 14 days."
            cards={cachedCards}
          />
